...
| 更新履歴 | 2025/07/17 : テーブル設計の修正 : Popcorn | 

## 1. はじめに

### 1.1. 本書の目的

本書は、ECサイト新規構築プロジェクトにおける詳細設計の内容を定義するものです。基本設計書 Ver.1.4 で定義された内容に基づき、実装担当者がプログラミング作業を迷いなく進められるように、システムの内部構造、処理フロー、インターフェース、データベース構造、画面項目などを具体的に記述します。
...
sequenceDiagram
    ...
    alt 商品が存在する場合
        ProductService->>ProductService: ProductエンティティからProductItem DTOへ変換
        ProductService-->>ProductController: ProductItem
...
sequenceDiagram
    ...
    alt カートが空でない場合
        ...
        alt 在庫あり
            ...
            OrderService->>OrderService: 注文情報(Order)・注文明細(OrderItem)エンティティ作成
            OrderService->>OrderRepository: save(order) // 注文情報保存
            OrderRepository->>DB: INSERT INTO orders ..., INSERT INTO order_items ...
...
classDiagram
    class ProductController {
        ...
        +getProductById(productId) ResponseEntity~ProductItem~
    }
    class ProductService {
        +ProductRepository productRepository
        +findAllProducts() List~ProductListItem~
        +findProductById(productId) ProductItem
    }
    ...
    ProductService ..> ProductItem : creates
    ProductController ..> ProductListItem : returns
    ProductController ..> ProductItem : returns
```
...
classDiagram
    class AdminController {
        ...
        +ProductService productService
        +showLogin(HttpSession, Model) String
        +login(Map~String,String~, HttpSession) ResponseEntity~?~
        +showDashboard(HttpSession, Model) String
        +showForgotPassword() String
        +showResetPassword(String, Model) String
        +logout(HttpSession) String
        +logoutApi(HttpSession) ResponseEntity~?~
        +getLoginStatus(HttpSession) ResponseEntity~?~
    }
    ...
    class ForgotPasswordController {
        ...
        +sendResetLink(Map~String,String~) ResponseEntity~?~
        +resetPassword(Map~String,String~) ResponseEntity~?~
    }
    ...
    class AdminService {
        ...
        +createAdmin(String, String, String, String) void
        +authenticate(String, String) Admin
        +findByUsername(String) Admin
        +save(Admin) Admin
        +changePassword(Admin, String) Admin
    }
    ...
    class AdminRepository {
        ...
        +findByUsernameAndActive(String, Boolean) Optional~Admin~
    }

    class PasswordResetTokenService {
        +Map~String,TokenInfo~ tokenStore
    ...
    class Admin {
        ...
        +String role
        +boolean isActive
        +LocalDateTime lastLoginAt "Nullable"
    ...
    class EmailService {
        <<External Service>>
        +sendResetLink(String, String) void
    }
    ...
    ForgotPasswordController --> AdminRepository : uses
    ForgotPasswordController --> EmailService : uses
    ForgotPasswordController --> PasswordResetTokenService : uses
    AdminService --> AdminRepository : uses
    AdminRepository "1" -- "*" Admin : manages
    
    AdminController ..> LoginRequest : receives
    AdminController ..> LoginResponse : returns
    AdminController ..> StatusResponse : returns
    AdminService ..> Admin : creates/manages
    ForgotPasswordController ..> Admin : updates password
```
...
- **DTO (Data Transfer Object)**: レイヤー間（特にControllerとService、APIの境界）でデータを転送するためのオブジェクトです。APIのリクエスト/レスポンス形式の定義にも使用されます。
    - 例: `ProductListItem`, `ProductItem`, `Cart`, `OrderRequest`, `OrderResponse`
...
### customers（顧客マスタ）
ログイン、新規登録機能は今回のスコープ外である。
今後の拡張性を考慮して、customersテーブルを作成。
 
| No | 項目論理名   | 項目物理名        | データ型     | 桁数  | NULL | PK | FK | UQ | CHK | INDEX | デフォルト                        | 備考       |
| -- | ------- | ------------ | -------- | --- | ---- | -- | -- | -- | --- | ----- | ---------------------------- | -------- |
| 1  | 顧客ID    | customer\_id | INT      |     | NO   | ○  |    | ○  |     | ○     | AUTO\_INCREMENT              | 一意の顧客ID  |
| 2  | 氏名      | name         | STRING  | 100 | NO   |    |    |    |     |       |                              |          |
| 3  | メールアドレス | customer\_email        | STRING  | 255 | NO   |    |    | ○  |     | ○     |                              | UNIQUE制約 |
| 4  | パスワード   | password     | STRING  | 255 | YES  |    |    |    |     |       |                              | ハッシュ化    |
...
| 2  | 商品名     | product\_name         | VARCHAR       | 100 | NO   |    |    |    |     |       |                 |
| 3  | 商品説明    | description  | STRING          |     | YES  |    |    |    |     |       |                 |
| 4  | 商品画像URL | image\_url   | STRING       | 255 | YES  |    |    |    |     |       |                 |
| 5  | カテゴリID  | category\_id | INT           |     | NO   |    | ○  |    |     | ○     | categories参照    |
| 6  | 価格      | price        | INT |     |NO   |    |    |    |     |       |                 |
| 7  |在庫数　　　　| stock | INT  | |NO  |    |     |     |      |     |     |
| 8  | 登録日時    | created\_at  | DATETIME      |     | NO   |    |    |    |     |       |                 |
| 9  | 更新日時    | updated\_at  | DATETIME      |     | NO   |    |    |    |     |       |                 |
| 10  | おすすめ     |  is\_recommended        |BOOLEAN       |  | NO   |    |    |    |     |       |                 |
...
| 2  | カテゴリ名  | category\_name | STRING  | 100 | NO   |    |    |    |     |       |    |
...
| No | 項目論理名   | 項目物理名             | データ型     | 桁数  | NULL | PK | FK | UQ | CHK | INDEX | 備考              |
| -- | ------- | ----------------- | -------- | --- | ---- | -- | -- | -- | --- | ----- | --------------- |
| 1  | 注文ID    | order\_id         | INT      |     | NO   | ○  |    |  ○   |     | ○     |  |      
| 2  | 合計金額    | total\_amount   | INT    | 50  | NO   |    |    |    |     |       |                 |
| 3  | 顧客名    | customer\_name   | STRING  | 50  | NO   |    |    |    |     |       |                 |
| 4  | 配送先住所   | shipping\_address | STRING  | 500 | NO   |    |    |    |     |       |                 |
| 5  | 配送先電話番号   | shipping\_phoneNumber | STRING  | 500 | NO   |    |    |    |     |       |                 |
| 6  | 注文ステータス | order\_status     | STRING  | 50  | NO   |    |    |    |     |       |                 |
| 7  | 注文日時    | order\_date       | DATETIME |     | NO   |    |    |    |     |       |                 |
| 8  | 登録日時    | created\_at       | DATETIME |     | NO   |    |    |    |     |       |                 |
| 9  | 更新日時    | updated\_at       | DATETIME |     | NO   |    |    |    |     |       |                 |
| 10  | 顧客ID    | customer\_id | INT      |     | NO   |   | ○   | ○  |     | ○      | 現在の実装のスコープ外  |
| 11  | メールアドレス | customer\_email        | STRING  | 255 | NO   |    |    | ○  |     | ○     |  現在の実装のスコープ外|
...
| 3  | 商品名     | product\_name         | VARCHAR       | 100 | NO   |    |    |    |     |       |   
| 4  | 商品ID   | product\_id     | INT           |    | NO   |    | ○  |    |     | ○     | products参照      |
| 5  | 数量     | quantity        | INT           |  15 | NO   |    |    |    |     |       |                 |
| 6  | 注文時単価  | price           | INT |    | NO   |    |    |    |     |       |                 |

 
### admin（管理者テーブル）
| No | 項目論理名  | 項目物理名           | データ型          | 桁数 | NULL | PK | FK | UQ | CHK | INDEX | 備考              |
| -- | ------ | --------------- | ------------- | -- | ---- | -- | -- | -- | --- | ----- | --------------- |
| 1  | 管理者氏名 | name | STRING           |    | NO   |    |    |    |     |      |管理者の個人名    |
| 2  | ユーザーネーム   | user\_name       |  STRING           |    | NO   |    |   |    |   | ○     |  ログイン時に必要な名前          |
| 3  | パスワード   | password     | STRING           |    | NO   |    |    |    |     |      |       |
| 4  | メールアドレス     | 　email        | STRING         |    | NO   |    |    |    |     |       |                 |
| 5  | ロール  |    role        | STRING |    | NO   |    |    |    |     |       |                 |
| 6  | 登録日時   | created\_at     | DATETIME      |    | NO   |    |    |    |     |       |                 |
| 7  | 更新日時   | updated\_at     | DATETIME      |    | NO   |    |    |    |     |       |                 |
| 8  | 管理者ID | id | INT           |    | NO   |  ○   |    |  ○   |    |○  | 内部で処理するためのもの|
| 9  | 有効 | is\_active | BOOLEAN           |    | NO   |    |    |    |     | 
| 10  | 最終ログイン日時 |  last\_login\_at| DATETIME           |    | NO   |    |    |    |     | 
