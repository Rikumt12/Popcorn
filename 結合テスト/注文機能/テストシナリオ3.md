...
- 前提条件:
  ...
  - セッションにカート `Cart(items={"1": CartItem(..., quantity=2, subtotal=7000), "2": CartItem(..., quantity=1, subtotal=8400)}, totalQuantity=3, totalPrice=15400)` が存在する。
...
- 前提条件:
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 3500, 3)` が存在する。（在庫が3）
  - セッションにカート `Cart(items={"1": CartItem(..., quantity=3, subtotal=10500)}, totalQuantity=3, totalPrice=10500)` が存在する。（注文数も3）
- 手順:
...
- 前提条件:
  ...
  - DBに商品 `Product(1, "シンプルデスクオーガナイザー", 500, 20)` が存在する。
